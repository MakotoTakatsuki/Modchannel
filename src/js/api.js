...
api.addIndicator = function(className, title, thread) {
  ...
  if (!spanId) {
    ...
    spanId = (thread || document).getElementsByClassName('extraMenuButton')[0];
  }

  if (className == "lockIndicator") {
	var indicator = document.createElement('img');
    indicator.src= "/.static/images/locked.gif";
    indicator.alt = "locked";
    indicator.className = className;
    indicator.title = title;
  } else if (className == "cyclicIndicator") {
	var indicator = document.createElement('img');
	indicator.src= "/.static/images/cyclic.png";
    indicator.alt = "cyclic";
    indicator.className = className;
    indicator.title = title;
  } else if (className == "pinIndicator") {
	var indicator = document.createElement('img');
	indicator.src= "/.static/images/sticky.gif";
    indicator.alt = "sticky";
    indicator.className = className;
    indicator.title = title;
  } else if (className == "bumpLockIndicator") {
	var indicator = document.createElement('span');
	indicator.textContent = "[AUTOSÄGE]"
	indicator.className = className;
	indicator.title = title;
  } else {
	var indicator = document.createElement('span');
...
api.handleConnectionResponse = function(xhr, callback, silent) {
  ...
  if (response.status === 'error') {
    ...
    if (response.data.reason) {
      message += '\nReason: "' + response.data.reason.replace('&gt;', '>') + '".';
    }
    ...
    if (!response.data.appealled) {
      ...
      if (appeal && appeal !== 'Write your appeal') {
...
api.formApiRequest = function(page, parameters, callback, silent, getParameters, showFailed) {

  var silent;

  if (page.indexOf('addon.js') < 0) {
    page += '.js?json=1';
  } else {
    page += '&json=1';
  }
...
if (document.cookie.indexOf("svg_support=") < 0) {
    setCookie("svg_support", "true");
}
